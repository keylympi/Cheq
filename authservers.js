async function checkAuth() {
  const url = "https://raw.githubusercontent.com/keylympi/Cheq/refs/heads/main/bhh.txt";

  try {
    const res = await fetch(url, { cache: "no-store" }); // no cache for freshness
    if (!res.ok) throw new Error("Network response not ok");

    const text = (await res.text()).trim().toLowerCase();

    if (text === "false") {
      alert("this script has been locked by the owner");
    ...
    if (text === "pass") {
      ...
      const correctHash = "2362c3a65507e96754f086d246a2fc187ccfba27bba11261118ef9682e9c6e36";
    ...
    console.error(err);
    alert("authenification servers cant be reached try again later");
  }
}

// Example "main script" function
function runScript() {
...
(() => {
    ...
    img.onerror = img.onabort = () => {
        ...
        cheat();
        let iframe = document.querySelector("iframe");
        iframe.contentWindow.alert("Bookit hacks, By 05Konz, DumpsterFire, DevShadowz, and")
    }
})();
}
